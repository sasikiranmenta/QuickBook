...
import com.sasi.quickbooks.model.invoice.InvoiceItem;
...
public class PDFGeneratorService {
    ...
    public static PdfPTable getDetailsTable(Invoice quickBookInvoice) throws DocumentException {
    ...
    public static PdfPTable getGstinTable(Invoice invoice) throws DocumentException {
        PdfPTable GstinTable = new PdfPTable(2);
        String idType = getIdType(invoice);
        float idColumnWidth = 0.05f * idType.length();
        float[] columnWidths = {idColumnWidth, 2f};
        GstinTable.setWidths(columnWidths);
        GstinTable.setWidthPercentage(95);

        GstinTable.addCell(PDFUtil.getCellLeftAlignNoBorder(idType));
        GstinTable.addCell(PDFUtil.getCellInputCellLeftAlignBottomBorderColored(getIdNumber(invoice)));
        return GstinTable;
    }

    public static PdfPTable getItemDetails(Invoice quickBookInvoice) throws DocumentException {
        PdfPTable itemDetailsTable = new PdfPTable(5);
        itemDetailsTable.setWidthPercentage(95);
        itemDetailsTable.setSpacingBefore(10f);
        int defaultNumItems = 10 - quickBookInvoice.getPaymentMode().size();
        ...
        itemDetailsTable.addCell(PDFUtil.getCellInputCellCenterAlignBottomAndRightBorderColored(quickBookInvoice.getInvoiceType().getName()));
    ...
    public static PdfPTable getAmountDetailsTable(Invoice quickBookInvoice) throws DocumentException {
    ...
    public static PdfPTable getInvoiceAndDateTable(Invoice quickBookInvoice) throws DocumentException {
    ...
    private static PdfPTable getPaymentModeDetailsTable(Invoice quickBookInvoice) throws DocumentException {
    ...
    public void generateInvoiceFile(Invoice quickBookInvoice, HttpServletResponse response) throws IOException, DocumentException {
        Document document = new Document(PageSize.A5, 1f, 1f, 1f, 0f);
    ...
    public void generateInvoices(List<Invoice> quickBookInvoices, HttpServletResponse response) throws IOException, DocumentException {
        Document document = new Document(PageSize.A5, 1f, 1f, 1f, 0f);
    ...
    private static String getIdType(Invoice invoice) {
        if(ObjectUtils.isEmpty(invoice.getIdentificationNumberType())) {
            return "GSTIN / PAN / AADHAAR";
        }
        return invoice.getIdentificationNumberType().toString();
    }

    private static String getIdNumber(Invoice invoice) {
        if(ObjectUtils.isEmpty(invoice.getIdentificationNumber())) {
            return "";
        }
        return invoice.getIdentificationNumber();
    }
